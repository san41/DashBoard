...
module.exports = function(socket){
	...
	// Count 
  socket.on('weather/count', function(callback){
    Weather.count({'user':socket.request.user}, callback);
  }); 	
  ...
  socket.on('weather/addCity', function(data, callback){
    // check if the city id is already on the database 
      ...
      // if data is not avaible on the database
  		if(dataFind == null){
  			var weather = new Weather({cityid : data.id, name : data.name, position : data.position});
